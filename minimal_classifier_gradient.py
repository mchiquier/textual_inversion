...
output_path = "results/whiskers_bptt_700_logper" 
...
#embedding = torch.load('results/whiskers/embeddings/embedding_13.pt')
model.embedding_manager.load('results/whiskers/embeddings/embedding_13.pt')
...
for i in range(8000):
    ...
    for j,raw_batch in enumerate(train_loader):
        ...
        #pdb.set_trace()
        samples, z_denoise_row = model.sample_log(cond=cond,batch_size=raw_batch['edited'].shape[0],ddim=False,ddim_steps=200,eta=1.0)
        ...
        if True: #True: #i % 1 == 0 and j % 100 == 0:
