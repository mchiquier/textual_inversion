...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        zx = encoder_posterior_x.mode() #.detach() #self.get_first_stage_encoding(encoder_posterior_x).detach()
        ...
        cond["c_crossattn"] = [torch.where(prompt_mask, null_prompt, xc["c_crossattn"])] #.detach()
        cond["c_concat"] = [input_mask * encoder_posterior_y.mode()] #.detach()

        out = [zx, cond]
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(zx)
            out.extend([x, xrec])
