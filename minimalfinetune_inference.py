...
num=100
model.embedding_manager.load('embeddings/inference_noperimgtoken_dropout_05_bs16_juststar_10vectors_cloudsstar_finetune/embedding_0_480.pt')
...
raw_batch['caption'] = ['*', '*', '*', '*','*', '*', '*', '*','*', '*', '*', '*','*', '*', '*', '*']
thestring="inference/inference_noperimgtoken_dropout_05_bs16_juststar_10vectors_cloudsstar_finetune_inference"
        ...
        if i%1==0 and j%1==0:
            ...
            for key in log.keys():
                ...
                if key =='reals':
                    ...
                    im.save(thestring + "/" + str(i) + "/" + str(j) + "/" + filename)
            #pdb.set_trace()
            #if not os.path.isdir("embeddings/" + thestring):
            #    os.mkdir("embeddings/" + thestring)
            #model.embedding_manager.save("embeddings/" + thestring + "/embedding_" + str(i) + "_" + str(j) + ".pt")
